...
// Auto-seed realistic demo data on first-ever launch if no transactions exist
if (typeof window !== 'undefined') {
    window.addEventListener('DOMContentLoaded', () => {
        try {
            if (typeof storage !== 'undefined') {
                const existing = storage.getAllTransactions();
                if (!existing || existing.length === 0) {
                    loadDemoData(true);
                }
            }
        } catch (e) {
            console.warn('Auto-seed demo data note:', e);
        }
    });
}
