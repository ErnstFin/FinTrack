    ...
    terbilang(angka) {
        ...
        const konversi = (n) => {
            ...
            if (n < 12) {
                ...
                return konversi(Math.floor(n / 1000000)) + " Juta" + (sisa ? " " + konversi(sisa) : "");
            } else if (n < 1000000000000) {
                const sisa = n % 1000000000;
                return konversi(Math.floor(n / 1000000000)) + " Miliar" + (sisa ? " " + konversi(sisa) : "");
            } else if (n < 1000000000000000) {
                const sisa = n % 1000000000000;
                return konversi(Math.floor(n / 1000000000000)) + " Triliun" + (sisa ? " " + konversi(sisa) : "");
            } else {
                return "Nominal Sangat Besar";
            }
        };

        const hasilTeks = konversi(angka).replace(/\s+/g, ' ').trim();
        return `${hasilTeks} Rupiah`;
    }
        ...
        const triggerDemoLoad = async () => {
            ...
            if (confirmed) {
                if (typeof window.loadDemoData === 'function') {
                    ...
                    this.showToast('Modul data demo sedang disiapkan...', 'info');
                }
            }
        };
